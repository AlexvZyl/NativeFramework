...
GLuint BaseEngineGL::loadBMPtoGL(int bitmapID)
{
	...
	stbi_set_flip_vertically_on_load(1);
	auto* pixels = stbi_load("Source\\Resources\\Fonts\\Arial_SDF.bmp", &width, &height, &bits, STBI_rgb_alpha);
	...
	//GLCall(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, BM.bmWidth, BM.bmHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, BM.bmBits));
	stbi_image_free(pixels);
