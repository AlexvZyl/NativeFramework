...
Polygon2D::Polygon2D(std::vector<glm::vec3>* vertices, VertexArrayObject* VAO)
{
	...
	if (m_VAO->m_bufferType == GL_TRIANGLES) {
		...
		{
			m_vertices.insert(m_vertices.end(), VertexData(&vertices[0][0],		 &m_colour, m_entityID));
			m_vertices.insert(m_vertices.end(),	VertexData(&vertices[0][i],		 &m_colour, m_entityID));
			m_vertices.insert(m_vertices.end(), VertexData(&vertices[0][i + 1],  &m_colour, m_entityID));
		}
	}
	else if (m_VAO->m_bufferType == GL_LINES) 
	{
		...
		// Close loop.
		m_vertices.insert(m_vertices.end(), VertexData(&vertices[0][n_vertices - 1], &m_colour, m_entityID));
		m_vertices.insert(m_vertices.end(), VertexData(&vertices[0][0],				 &m_colour, m_entityID));
	}
