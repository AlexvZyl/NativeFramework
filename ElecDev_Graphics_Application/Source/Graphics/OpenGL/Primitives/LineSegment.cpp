...
void LineSegment::setStart(const glm::vec2& start)
{
	...
	m_VAO->m_vertexCPU[m_vertexBufferPos].data.position = glm::vec3(m_start - m_thickness / 2.0f * (m_perpendicular::member), 0.f);
	m_VAO->m_vertexCPU[m_vertexBufferPos + 1].data.position = glm::vec3(m_start + m_thickness / 2.0f * (m_perpendicular::member), 0.f);
	m_VAO->m_vertexCPU[m_vertexBufferPos + 2].data.position = glm::vec3(m_end + m_thickness / 2.0f * (m_perpendicular::member), 0.f);
	m_VAO->m_vertexCPU[m_vertexBufferPos + 3].data.position = glm::vec3(m_end - m_thickness / 2.0f * (m_perpendicular::member), 0.f);
	syncWithGPU();
}
