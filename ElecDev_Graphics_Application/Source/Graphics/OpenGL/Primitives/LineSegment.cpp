...
void LineSegment::translate(const glm::vec2& translation)
{
	...
	for (int i = 0; i < m_vertexCount; i++) getVertex(i).data.position += translation3;
...
void LineSegment::setStart(const glm::vec2& start)
{
	...
	getVertex(0).data.position = glm::vec3(m_start - m_thickness / 2.0f * (m_perpendicular::member), 0.f);
	getVertex(1).data.position = glm::vec3(m_start + m_thickness / 2.0f * (m_perpendicular::member), 0.f);
	getVertex(2).data.position = glm::vec3(m_end + m_thickness   / 2.0f * (m_perpendicular::member), 0.f);
	getVertex(3).data.position = glm::vec3(m_end - m_thickness   / 2.0f * (m_perpendicular::member), 0.f);
	syncWithGPU();
}
...
{
	// Check if we should move the end or the start of the line.
	if (glm::length(glm::vec2(getVertex(index).data.position) - m_end) < 
		glm::length(glm::vec2(getVertex(index).data.position) - m_start)) 
	{
		// Move the end.
		setEnd(m_end + translation);
	}
