...
#define DEFREELIST_MAX_CAPACITY 2147483648 // 2^31 for int.

template<typename T>
class DEFreeList : public FreeList<T>
{
...
public:
	...
	// Constructors.
	DEFreeList() : FreeList() { }
	DEFreeList(int capacity) : FreeList(capacity) { }
	DEFreeList(int capacity, int capacityIncrements) : FreeList(capacity, capacityIncrements) { }

	// Copy constructor.
	DEFreeList(const DEFreeList& other) : FreeList(DEFreeList) { }
